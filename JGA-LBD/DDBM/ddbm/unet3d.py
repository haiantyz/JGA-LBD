...
from .nn2 import avg_pool_nd, conv_nd, linear, normalization, timestep_embedding, zero_module, get_activation, \
    AttentionPooling

_FORCE_MEM_EFFICIENT_ATTN = int(os.environ.get('FORCE_MEM_EFFICIENT_ATTN', 1))
print('FORCE_MEM_EFFICIENT_ATTN=', _FORCE_MEM_EFFICIENT_ATTN, '@UNET:QKVATTENTION')
_FORCE_MEM_EFFICIENT_ATTN=True
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, xT=None, smpl=None, cond_text=None):
        # print(timesteps)
        x = torch.cat([x,xT,smpl],1)
        hs = []
        input_type = x.dtype
