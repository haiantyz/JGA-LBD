...
def save_sparse_tensor_with_scipy(tensor: torch.Tensor, save_path: str, occupancy_threshold: float = 0.9):
    """
    将输入 tensor 根据 occupancy 通道筛选后，使用 scipy.sparse 保存为稀疏格式。

    参数:
        tensor (torch.Tensor): 输入 tensor，shape [7, D, H, W]。
        save_path (str): 保存路径 (.npz 文件)。
        occupancy_threshold (float): 判定 occupancy 有效的阈值，默认 0.9。
    """
    # 检查输入
    assert tensor.dim() == 4 and tensor.shape[0] == 9, "输入 tensor 的 shape 必须是 [7, D, H, W]"

    # occupancy 通道是最后一个
    occupancy = tensor[-1]  # shape [D, H, W]
    
    # 筛选 occupancy > 阈值的点
    mask = occupancy > occupancy_threshold  # bool mask

    # 获取符合条件的网格坐标
    coords = mask.nonzero(as_tuple=False)  # [N, 3] (D, H, W)
    
    # 获取对应的特征（前6维）
    features = tensor[:-1]  # shape [6, D, H, W]
    features_flat = features.view(8, -1)  # [6, D*H*W]
    mask_flat = mask.view(-1)             # [D*H*W]
    selected_features = features_flat[:, mask_flat]  # [6, N]
    selected_features = selected_features.t()  # [N, 6]

    # 将 coords 和 features 转为 numpy
    coords_np = coords.cpu().numpy()
    features_np = selected_features.cpu().numpy()

    # 用 scipy.sparse 保存成稀疏矩阵
    # 把 coords 和 features 合成一个稀疏矩阵存储
    N = coords_np.shape[0]
    coords_features = np.hstack([coords_np, features_np])  # [N, 9] -> 3 coords + 6 features

    # 构建稀疏矩阵（这里使用COO格式，每一行是一个有效点及其特征）
    sparse_matrix = sp.coo_matrix(coords_features)
...
def main():
    ...
    for i, data in enumerate(dataloader):
        ...
        smpl = data[2].to(dist_util.dev())
        image_cond = data[3].to(dist_util.dev())
        model_kwargs = {'xT': y0, 'smpl':smpl, 'cond_text': image_cond}
        ...
        print(sample.max())
        print(sample.min())
        # np.save("/data/tangyingzhi/Project/DDBM/ddbmresult.npy", sample.detach().cpu().numpy())
        ...
        all_images=(sample.detach().cpu())
        print(all_images.shape)
        # all_images_array = np.array(all_images)
        for batch_id in range(12):
            save_sparse_tensor_with_scipy(all_images[batch_id],"/data/tangyingzhi/Project/DDBM/onlydepth/new"+str(i)+"_"+str(batch_id)+".npz")
        # np.save("/data/tangyingzhi/Project/DDBM/ddbmnewve.npy", all_images)
