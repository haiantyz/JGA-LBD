...
def load_data(
  ...
  if dataset == 'edges2handbags':
    ...
    from .aligned_dataset import Latent3DGSDataset
    trainset = Latent3DGSDataset(train=True)
    valset = Latent3DGSDataset(train=True)
    
    if include_test:
      print("this is test dataset")
      testset = Latent3DGSDataset(train=False)
  elif dataset == 'diode':
