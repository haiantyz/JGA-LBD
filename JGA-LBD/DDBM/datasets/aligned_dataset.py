...
class EdgesDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataroot, train=True,  img_size=264, random_crop=False, random_flip=True):
...
class Latent3DGSDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        person = self.file_list[idx]
        
        
        partial_condtion = sparse.load_npz(""+person+".npz").toarray().reshape(9, 64, 64, 64)
        smpl_condtion = sparse.load_npz(""+person+".npz").toarray().reshape(9, 64, 64, 64)
       
        partial_condtion = torch.tensor(partial_condtion, dtype=torch.float32).reshape(9, 64, 64, 64)
        smpl_condtion = torch.tensor(smpl_condtion, dtype=torch.float32).reshape(9, 64, 64, 64)
        
        volume = torch.randn(9, 64, 64, 64)
        return volume, partial_condtion, partial_condtion, 0     
...
class DIODE(torch.utils.data.Dataset):
    ...
    def __init__(self, dataroot, train=True,  img_size=264, random_crop=False, random_flip=True, down_sample_img_size = 0, cache_name='cache', disable_cache=False):
        """Initialize this dataset class.
        Parameters:
            opt (Option class) -- stores all the experiment flags; needs to be a subclass of BaseOptions
        """
        ...
        if os.path.exists(self.cache_path) and not disable_cache:
            ...
            # self.cache['img'] = self.cache['img'][:264]
